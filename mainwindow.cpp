    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->setupUi(this);
    ui->save_checkBox->setEnabled(false); //不保存数据
    hWnd = (HWND)this->winId();


    string path = "C:/DAS/Config.txt";
    cfg = make_shared<Config>(); //Config对象的指针
    cfg->Init(path);
    getcfg = make_shared<GetConfig>(*cfg); //获取Config对象
    MinWignoreDevice = getcfg->getConfig_Debug(); //是否debug
    CaculationPhase = getcfg->getConfig_calcPhase(); //是否解调相位
    CHDataQue = make_shared<CirQueue<float>>(getcfg->getConfig_freqency()* 3 * 600);//循环队列长度为原始通道（3个）数据10min的数据量
    demoduDataQue = make_shared<CirQueue<float>>(getcfg->getConfig_freqency()* 3 * 600);//循环队列长度为原始通道（3个）数据10min的数据量
    WaveDataQue = make_shared<CirQueue<float>>(getcfg->getConfig_freqency()* 3 * 600);//循环队列长度为原始通道（3个）数据10min的数据量
    setSENDSIZE( (int)(getcfg->getConfig_freqency() / 4) );//根据采样率初始化单次发送数据包大小
    if( MinWignoreDevice == true ) //debug模式
    {
...
/**
 * @brief MainWindow::setShow 是否显示波形
 * @return
 */
...
/**
 * @brief MainWindow::setSENDSIZE 设置发送数据大小
 * @param sendsize
 * @return
 */
...
/**
 * @brief MainWindow::on_Eazystart_Button_clicked 点击快速解调按钮
 */
...
/**
 * @brief MainWindow::on_SearchPeak_Button_clicked 开始寻峰 按钮按下
 */
...
/**
 * @brief MainWindow::nativeEvent 本地事件的相关操作 用于检查bug
...
/**
 * @brief MainWindow::on_WatchPeak_Button_clicked 查看峰值 按钮按下
 */
...
void MainWindow::ConfirmPeakResult()
{
    ...
    {
        SendPeakPosTimer = new QTimer();//设定一个QTimer用于定时读取peak.txt的值
        sendPeakChannel = 0;
    ...
    peak->savePeakNum(getcfg->getPointoConfig(), MinWignoreDevice);//设置peakNum
...
void MainWindow::debugDemudu(bool)
{
    ...
    //RVD.get()返回该指针 然后发送Speed到ShowSpeed
...
/**
 * @brief MainWindow::on_Demodu_Button_clicked 开始解调 按键按下
 */
...
/**
 * @brief MainWindow::on_DemoduStop_Button_clicked 解调停止 按钮按下
 */
...
/**
 * @brief MainWindow::on_Play_Button_clicked 声音录制 按钮按下
 */
...
/**
 * @brief MainWindow::on_save_checkBox_clicked 数据存储 复选框选中
 */
void MainWindow::on_save_checkBox_clicked()
{
    if (ui->save_checkBox->isChecked())
    {
        ...
        connect(&UDP->saveTimer,&QTimer::timeout,UDP,&UDPConnect::getFilename);
...
/**
 * @brief MainWindow::on_Sound_Button_clicked 播放 按钮按下
 */
